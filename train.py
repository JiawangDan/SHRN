...
def main(cfg):
    ...
    open_type = 'a' if os.path.exists(cfg.ckpt_dir + '/log.txt') else 'w'
    log_file = open(cfg.ckpt_dir + '/log.txt', open_type)

    # dynamic import using --model argument
    net = importlib.import_module("model.{}".format(cfg.model)).Net
    print(json.dumps(vars(cfg), indent=4, sort_keys=True))
    log_file.write(json.dumps(vars(cfg), indent=4, sort_keys=True) + '\n')

    solver = Solver(net, cfg, log_file)
    solver.fit()
